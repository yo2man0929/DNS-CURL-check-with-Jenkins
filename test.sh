...
check_http_service() {
  local url=$1
  local initial_status_code
  local final_url

  initial_status_code=$(curl -s -o /dev/null -w '%{http_code}' --max-time 5 "$url")
  final_url=$(curl -Ls -o /dev/null -w '%{url_effective}' --max-time 5 "$url")

  # 如果初始状态码是3xx，说明有重定向
  if [ "$initial_status_code" -ge 300 ] && [ "$initial_status_code" -lt 400 ]; then
    echo "${initial_status_code}|REDIRECT|${final_url}"
  else
    echo "${initial_status_code}|NO_REDIRECT|${final_url}"
  fi
}
...
while IFS= read -r url; do
  ...
  redirect_status=$(echo "$result" | cut -d'|' -f2)
  final_url=$(echo "$result" | cut -d'|' -f3)

  dns_status=$(check_dns_resolution "$url")
  line="${url}_curl: ${http_status}, ${url}_dns: ${dns_status}"

  if [ "$redirect_status" = "REDIRECT" ]; then
    redirect_http_status=$(curl -s -o /dev/null -w '%{http_code}' --max-time 5 "$final_url")
...
for domain in $CHECKING_DOMAIN; do
  if [ -n "$domain" ]; then
    ...
    http_status_and_info=$(check_http_service "$domain")
    initial_http_status=$(echo "$http_status_and_info" | cut -d'|' -f1)
    redirect_info=$(echo "$http_status_and_info" | cut -d'|' -f2)
    final_url=$(echo "$http_status_and_info" | cut -d'|' -f3)

    dns_status=$(check_dns_resolution "$domain")
    redirect_dns_status="not_applicable"
    redirect_curl_status="not_applicable"

    line="${domain}_curl: ${initial_http_status}, ${domain}_dns: ${dns_status}"
